...
const TYPE_PRIORITY = { programming: 1, markup: 2, data: 3, prose: 4 };
// 最大扫描字节，超过则跳过语言分析（可通过环境变量 CODENEST_MAX_SCAN_BYTES 覆盖，默认 800MB）
const MAX_SCAN_BYTES = Number(process.env.CODENEST_MAX_SCAN_BYTES ?? 800 * 1024 * 1024);
function sortByMainLanguage(results) {
...
function toLangGroup(sortedEntries) {
...
async function getDirectorySizeCapped(dir, cap) {
    let total = 0;
...
async function listImmediateSubDirs(root) {
    try {
...
async function run() {
    ...
    if (payload.rootsEnabled) {
        ...
        for (const root of payload.roots) {
            (await listImmediateSubDirs(root)).forEach(dir => dirsToScan.add({ path: dir, ide: null }));
        }
    }
    if (payload.ideEnabled) {
        ...
        if (payload.jetbrains.enabled) {
            ...
            results.forEach(result => dirsToScan.add({ path: result.path, ide: result.ide }));
        }
        if (payload.vscode.enabled) {
            // VSCode
            const dbPath = payload.vscode.stateDbPath;
            const results = collectFromVscode(dbPath || null);
            results.forEach(result => dirsToScan.add({ path: result.path, ide: 'visual-studio-code' }));
        }
    }
    try {
        for (const { path: full, ide } of dirsToScan) {
            ...
            try {
                ...
                if (dirSize > MAX_SCAN_BYTES) {
                    parentPort?.postMessage({ type: 'item', item: { path: full, name, ide } });
                    continue;
                }
                ...
                const mainLangColor = sorted[0]?.[1]?.color;
                const langGroup = toLangGroup(sorted);
                parentPort?.postMessage({ type: 'item', item: { path: full, name, mainLang, mainLangColor, langGroup, ide } });
            }
            catch (e) {
                parentPort?.postMessage({ type: 'item', item: { path: full, name, error: e?.message || String(e) } });
            }
        }
