...
// 保存数据到指定的 JSON 文件
export async function saveData(fileType, data) {
...
// 从指定的 JSON 文件加载数据
export async function loadData(fileType) {
    const filePath = getDataFilePath(fileType);
    try {
...
// 打开指定的 JSON 文件
export async function openData(fileType) {
    const filePath = getDataFilePath(fileType);
    try {
        if (!fs.existsSync(filePath)) {
...
// 删除指定的 JSON 文件
export async function deleteData(fileType) {
    const filePath = getDataFilePath(fileType);
    try {
