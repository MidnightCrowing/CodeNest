...
let nextSessionId = 1;
export function start(event, payload) {
...
export function detectVscodeStateDbPath() {
    ...
    switch (process.platform) {
        case 'win32':
            ...
            break;
        case 'darwin':
            dbPath = path.join(app.getPath('home'), 'Library', 'Application Support', 'Code', 'User', 'globalStorage', 'state.vscdb');
            break;
        default: // linux
            dbPath = path.join(app.getPath('home'), '.config', 'Code', 'User', 'globalStorage', 'state.vscdb');
    }
